...
async def next_page(bot, query):
    ...
    if settings.get('button'):
        ...
        btn.insert(0, [
            InlineKeyboardButton(f"🎬 {search.title()} 🎬", callback_data="ident")
        ])
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(0, [
        InlineKeyboardButton(f"🎬 {search_final.title()} 🎬", callback_data="ident")
    ])
    if n_offset != "":
        try:
            if settings['max_btn']:
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
    ...
    elif query.data == "search_instruction":
        await query.answer("Here is how to search!", show_alert=False)
        ...
        buttons = [[
                    InlineKeyboardButton('🔎 SEARCH MOVIES OR SERIES 🔍', callback_data='search_instruction')
                ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    async def _schedule_delete(sent_obj, orig_msg, delay):
        try:
            # Extract details before sleep to ensure availability
            chat_id = orig_msg.chat.id
            try:
                user_mention = orig_msg.from_user.mention
    ...
    try:
        ...
        try:
            if settings.get('auto_delete'):
                asyncio.create_task(_schedule_delete(sent, message, BUTTON_DELETE_TIME))
        except KeyError:
            try:
